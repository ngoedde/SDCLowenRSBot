...
namespace RSBot.Core.Network.SecurityAPI
{
    internal class SecurityProtocol
    {
        ...
        private void Handshake(ushort packet_opcode, PacketReader packet_data, bool packet_encrypted)
        {
            if (packet_encrypted)
            {
                throw (new HandshakeSecurityException("[SecurityAPI::Handshake] Received an illogical (encrypted) handshake packet."));
            }
            if (m_client_security)
            {
                ...
                if (m_security_flags.handshake == 0)
                {
                    ...
                    if (packet_opcode == 0x9000)
                    {
                        ...
                        {
                            throw (new HandshakeSecurityException("[SecurityAPI::Handshake] Received an illogical handshake packet (duplicate 0x9000)."));
                        }
                    ...
                    {
                        throw (new HandshakeSecurityException("[SecurityAPI::Handshake] Received an illogical handshake packet (0x5000 with no handshake)."));
                    }
                    // Programmer made a mistake in calling this function
                    else
                    {
                        throw (new HandshakeSecurityException("[SecurityAPI::Handshake] Received an illogical handshake packet (programmer error)."));
                    }
                }
                else
                {
                    ...
                    if (packet_opcode == 0x9000)
                    {
                        ...
                        {
                            throw (new HandshakeSecurityException("[SecurityAPI::Handshake] Received an illogical handshake packet (out of order 0x9000)."));
                        }
                    ...
                    else if (packet_opcode == 0x5000)
                    {
                        ...
                        {
                            throw (new HandshakeSecurityException("[SecurityAPI::Handshake] Received an illogical handshake packet (duplicate 0x5000)."));
                        }
                    ...
                    foreach (TransferBuffer buffer in incoming_buffers_tmp)
                    {
                        ...
                        else
                        {
                            if (m_client_security)
                            {
                                ...
                                if (!m_accepted_handshake)
                                {
                                    throw (new HandshakeSecurityException("[SecurityAPI::Recv] The client has not accepted the handshake."));
                                }
                            }
                            if (packet_opcode == 0x600D) // Auto process massive messages for the user
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        throw (new HandshakeSecurityException("[SecurityAPI::Recv] A malformed 0x600D packet was received."));
                                    }
