...
namespace RSBot.Core.Client
{
    public class DDSImage
    {
        ...
        private static int[] A8B8G8R8_MASKS = { 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF0000 << 8 };
        private static int[] A8R8G8B8_MASKS = { 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF0000 << 8 };
        ...
        public static int[] Read(byte[] buffer, Colour color, int mipmapLevel)
        {
            ...
            int mipmap = (buffer[28] & 0xFF) | (buffer[29] & 0xFF) << 8 | (buffer[30] & 0xFF) << 16 | (buffer[31] & 0xFF) << 24;
            ...
            switch (type)
            {
                case DXT1: pixels = decodeDXT1(width, height, offset, buffer, color); break;
                case DXT2: pixels = decodeDXT2(width, height, offset, buffer, color); break;
                case DXT3: pixels = decodeDXT3(width, height, offset, buffer, color); break;
                case DXT4: pixels = decodeDXT4(width, height, offset, buffer, color); break;
                case DXT5: pixels = decodeDXT5(width, height, offset, buffer, color); break;
                case A1R5G5B5: pixels = readA1R5G5B5(width, height, offset, buffer, color); break;
                case X1R5G5B5: pixels = readX1R5G5B5(width, height, offset, buffer, color); break;
                case A4R4G4B4: pixels = readA4R4G4B4(width, height, offset, buffer, color); break;
                case X4R4G4B4: pixels = readX4R4G4B4(width, height, offset, buffer, color); break;
                case R5G6B5: pixels = readR5G6B5(width, height, offset, buffer, color); break;
                case R8G8B8: pixels = readR8G8B8(width, height, offset, buffer, color); break;
                case A8B8G8R8: pixels = readA8B8G8R8(width, height, offset, buffer, color); break;
                case X8B8G8R8: pixels = readX8B8G8R8(width, height, offset, buffer, color); break;
                case A8R8G8B8: pixels = readA8R8G8B8(width, height, offset, buffer, color); break;
                case X8R8G8B8: pixels = readX8R8G8B8(width, height, offset, buffer, color); break;
            }
            return pixels;
        }
            ...
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    ...
                    for (int k = 0; k < 4; k++)
                    {
                        ...
                        pixels[4 * width * i + 4 * j + width * k + 0] = getDXTColor(c0, c1, getDXT5Alpha(a0, a1, alphaTable[4 * k + 0]), t0, color);
                        if (4 * j + 1 >= width)
                            continue;

                        pixels[4 * width * i + 4 * j + width * k + 1] = getDXTColor(c0, c1, getDXT5Alpha(a0, a1, alphaTable[4 * k + 1]), t1, color);
                        if (4 * j + 2 >= width)
                            continue;

                        pixels[4 * width * i + 4 * j + width * k + 2] = getDXTColor(c0, c1, getDXT5Alpha(a0, a1, alphaTable[4 * k + 2]), t2, color);
                        if (4 * j + 3 >= width)
                            continue;

                        pixels[4 * width * i + 4 * j + width * k + 3] = getDXTColor(c0, c1, getDXT5Alpha(a0, a1, alphaTable[4 * k + 3]), t3, color);
                    }
                }
            }
            return pixels;
        ...
        private static int GetType(byte[] buffer)
        {
            ...
            int flags = (buffer[80] & 0xFF) | (buffer[81] & 0xFF) << 8 | (buffer[82] & 0xFF) << 16 | (buffer[83] & 0xFF) << 24;

            if ((flags & 0x04) != 0)
            {
                // DXT
                type = (buffer[84] & 0xFF) << 24 | (buffer[85] & 0xFF) << 16 | (buffer[86] & 0xFF) << 8 | (buffer[87] & 0xFF);
            }
            else if ((flags & 0x40) != 0)
            {
                // RGB
                int bitCount = (buffer[88] & 0xFF) | (buffer[89] & 0xFF) << 8 | (buffer[90] & 0xFF) << 16 | (buffer[91] & 0xFF) << 24;
                int redMask = (buffer[92] & 0xFF) | (buffer[93] & 0xFF) << 8 | (buffer[94] & 0xFF) << 16 | (buffer[95] & 0xFF) << 24;
                int greenMask = (buffer[96] & 0xFF) | (buffer[97] & 0xFF) << 8 | (buffer[98] & 0xFF) << 16 | (buffer[99] & 0xFF) << 24;
                int blueMask = (buffer[100] & 0xFF) | (buffer[101] & 0xFF) << 8 | (buffer[102] & 0xFF) << 16 | (buffer[103] & 0xFF) << 24;
                int alphaMask = ((flags & 0x01) != 0) ? (buffer[104] & 0xFF) | (buffer[105] & 0xFF) << 8 | (buffer[106] & 0xFF) << 16 | (buffer[107] & 0xFF) << 24 : 0; // 0x01 alpha
